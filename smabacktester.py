...
class SMABacktester():
    ...
    def get_data(self):
        ...
        def get_data(self):
        df = yf.download(self.symbol, start=self.start, end=self.end, progress=True)
        if df is None or df.empty:
            raise ValueError(f"Failed to download data for {self.symbol}. "
                             f"Check ticker, dates, internet, or upgrade yfinance.")
        if isinstance(df.Close, pd.Series):
            data = df.Close.to_frame(name="Close")
        else:
            data = df[["Close"]].copy()
